...
```
이 스텝에서는 스레드를 바인딩하고, Loader/Realm을 시작한다. try-catch-resource 구문이 너무 길어서 좀 잘라서 설명해야할 것 같다.

### 4-1. Loader 시작 + 웹액 ClassLoader 튜닝
```java
ClassLoader oldCCL = bindThread();
...
```
Tomcat은 하나의 Host 아래 여러 개의 Context(웹 애플리케이션)를 가질 수 있다. 각 Context는 독립적인 WebappClassLoader를 사용하여 애플리케이션 간 클래스 로딩을 격리한다.

그러나 Tomcat 내부에서는 스레드 풀(Executor)을 공유하므로, 요청 처리 스레드가 재사용될 수 있다. 따라서 StandardContext의 구동 과정에서 bindThread()를 호출하여
현재 스레드의 ContextClassLoader를 해당 웹앱의 WebappClassLoader로 바인딩하고, 실행이 끝난 뒤에는 unbindThread()를 통해 원래의 클래스로더로 복원한다.

이 과정을 통해 스레드 재사용 시 app1이 app2의 클래스로더를 참조하는 문제를 방지하며, 각 웹앱의 클래스/리소스가 완전히 격리된 환경에서 동작하도록 보장한다.
...
try {
            if (ok) {
                ...
                oldCCL = bindThread();
```
이렇게 획득한 ClassLoader는 WebappClassLoaderBase에 해당한다. WebappClassLoaderBase의 역할은 각 웹애플리케이션 전용 ClassLoader의 구현체 베이스 클래스이다.

`/WEB-INF/classess` 나 `/WEB-INF/lib/*.jar` 에 있는 것들을 한 웹앱 단위로 로드하는 클래스로더역할을 수행한다.
```scss
Bootstrap ClassLoader (JDK 기본)
  ↓
System / App ClassLoader (톰캣 실행 클래스)
  ↓
Common ClassLoader (톰캣 공용 라이브러리)
  ↓
WebappClassLoaderBase (각 컨텍스트별 웹앱용 로더)
```
이후 획득된 WebappClassLoaderBase 인스턴스에는 메모리 릭 방지를 위한 다양한 정책 메서드가 설정된다.

여기서 말하는 메모리 릭 방지란, 톰캣 위에 배포된 웹 애플리케이션이 컨텍스트 종료 시점에 적절히 정리하지 못한 스레드나 유휴 리소스를 톰캣이 대신 정리하여 클래스 로더가 GC에 정상적으로 회수되도록 하는 동작을 말한다.

예를 들어,
- setClearReferencesStopThreads()는 컨텍스트 종료 시 **아직 종료되지 않은 작업 스레드(Work Thread)**를 찾아 중단시키고,
- setClearReferencesStopTimerThreads()는 java.util.Timer로 생성된 TimerThread가 cancel() 되지 않은 경우 이를 탐지해 종료한다.

즉, 웹 애플리케이션 개발자나 라이브러리가 명시적으로 해제하지 않은 리소스들을 WebappClassLoaderBase가 컨텍스트 종료 시 보조적으로 정리함으로써,
재배포(redeploy)나 서버 종료 과정에서 발생할 수 있는 클래스로더 메모리 릭을 예방하는 역할을 수행한다.

### 4-2. Realm 시작 + CredentialHandler 공개
```java
...
```
Realm 은 Tomcat의 인증/인가 를 수행하는 추상화 컴포넌트이다.(JDBCRealm, DataSourceRealm, JNDIRealm 등 구체 클래스)

각 정의 및 주입된 구체 클래스를 실제로 시작시키는 작업을 수행한다.

(가령, JDBCRealm을 사용한다면, DB 커넥션 풀을 초기화하는 작업이 수행 된다.)

이후, CredentialHandler를 웹앱에 공개하는 작업을 수행한다.

`CredentialHandler` 인터페이스는 Realm이 비밀번호를 검증/해시할 때 사용하는 전략 객체이다.

SpringSecurity에서 PasswordEncoder와 동일한 역할을 수행한다고 보면 된다.